       ...
       $password = $json->getVar('password');
        ...
        public function sendResetEmail()
        {
            try {
                ...
                $phpMailer->Body = 'Click the link to reset your password: http://localhost:8080/updatePassword';
                $phpMailer->send();
        
                return $this->respond(['id' => (int) $data['id'], 'message' => 'Email sent! Check your email for verification']);
        ...
        public function sendResetEmailAdmin()
        {
            try {
                ...
                $phpMailer->addAddress($email);
                $phpMailer->Subject = 'Password Reset';
                $phpMailer->Body = 'Click the link to reset your password: http://localhost:8080/updatePass';
                $phpMailer->send();
